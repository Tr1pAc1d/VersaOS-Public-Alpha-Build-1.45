...
const nodesCode = allData.map(n => JSON.stringify(n)).join(',\n  ');

const vfsPath = path.join('d:', 'VersaOS-main', 'src', 'hooks', 'useVFS.ts');
let content = fs.readFileSync(vfsPath, 'utf8');

// Match the sys_log_04 line more loosely
const lines = content.split('\n');
const logLineIndex = lines.findIndex(l => l.includes('sys_log_04'));

if (logLineIndex !== -1) {
    lines[logLineIndex] = lines[logLineIndex] + "\n  " + nodesCode + ",";
    fs.writeFileSync(vfsPath, lines.join('\n'));
    console.log('Successfully updated useVFS.ts');
} else {
    console.error('Could not find sys_log_04 line in useVFS.ts');
}
